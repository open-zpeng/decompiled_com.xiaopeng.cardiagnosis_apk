...
public final class HostNameVerifier implements HostnameVerifier {
    ...
    public boolean verifyHostname(String hostname, String pattern) {
        if (hostname == null || hostname.length() == 0 || hostname.startsWith(Constant.DOT_STRING) || hostname.endsWith("..") || pattern == null || pattern.length() == 0 || pattern.startsWith(Constant.DOT_STRING) || pattern.endsWith("..")) {
            return false;
        }
        if (!hostname.endsWith(Constant.DOT_STRING)) {
            hostname = hostname + '.';
        }
        if (!pattern.endsWith(Constant.DOT_STRING)) {
            pattern = pattern + '.';
        }
