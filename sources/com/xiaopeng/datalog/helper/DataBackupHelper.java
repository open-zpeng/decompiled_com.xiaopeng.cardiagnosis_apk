...
public class DataBackupHelper {
    ...
    public void updateFile(final String tag, final String data, final Context context) {
        ThreadUtils.post(0, new Runnable() { // from class: com.xiaopeng.datalog.helper.DataBackupHelper.1
            ...
            public void run() {
                ...
                jsonObject.addProperty(PsuKeyModel.RESPONSE_MESSAGE, data);
    ...
    public void initDir(String tag, Context context) {
        File datafileDir = new File("/sdcard/Log/" + tag + InternalZipConstants.ZIP_FILE_SEPARATOR + context.getPackageName());
    ...
    public void deleteExpiredFile(final String tag, Context context) {
        File canFileDir = new File("/sdcard/Log/" + tag + InternalZipConstants.ZIP_FILE_SEPARATOR + context.getPackageName());
        ...
        for (String fileName : fileNames) {
            if (isExpiredFile(fileName)) {
                ...
                boolean isDeleted = new File("/sdcard/Log/" + tag + InternalZipConstants.ZIP_FILE_SEPARATOR + context.getPackageName() + InternalZipConstants.ZIP_FILE_SEPARATOR + fileName).delete();
    ...
    private boolean isExpiredFile(String fileName) {
        int indexEnd = fileName.lastIndexOf(Constant.DATA_SEPARATOR_STRING);
    ...
    private boolean hasTodayFile(File currentFile) {
        ...
        int index = dateStr.indexOf(Constant.DATA_SEPARATOR_STRING);
    ...
    private File createNewFile(String tag, Context context) {
        String dateStr = DateUtils.formatDate10(System.currentTimeMillis());
        return new File("/sdcard/Log/" + tag + InternalZipConstants.ZIP_FILE_SEPARATOR + context.getPackageName() + InternalZipConstants.ZIP_FILE_SEPARATOR + tag + Constant.DATA_SEPARATOR_STRING + dateStr + ".log");
    }
    ...
    private String getTimeFromFile(String fileName) {
        int index = fileName.lastIndexOf(Constant.DATA_SEPARATOR_STRING);
        int indexBegin = index - 8;
        int indexEnd = fileName.lastIndexOf(Constant.DOT_STRING);
