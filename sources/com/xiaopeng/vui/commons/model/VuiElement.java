...
public class VuiElement {
    ...
    public static JsonObject getElementAction(String actionStr) {
        ...
        for (int i = 0; i < actionStrs.length; i++) {
            if (actionStrs[i].contains(Constant.COLON_STRING)) {
                int index = actionStrs[i].indexOf(Constant.COLON_STRING);
                String name = actionStrs[i].substring(0, index);
                if (!actionEnumList.contains(name)) {
                    return null;
                }
                String[] actExtends = actionStrs[i].substring(index + 1).split(Constant.SEPARATOR_STRING);
